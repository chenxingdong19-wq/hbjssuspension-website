...
export default function ProductGallery({ images, name }: { images: string[]; name: string }) {
  ...
  const btnClass =
    "w-10 h-10 rounded-full glass-card flex items-center justify-center text-slate-500 hover:text-slate-800 transition-colors";
    ...
    <>
      ...
      <div className="space-y-4">
        ...
        <div className="relative aspect-square glass-card glass-noise overflow-hidden group">
          ...
          {images.length > 1 && (
            <>
              <button onClick={prev} className={`absolute left-3 top-1/2 -translate-y-1/2 z-10 opacity-0 group-hover:opacity-100 transition-opacity ${btnClass}`}>
                <ChevronLeft size={20} />
              </button>
              <button onClick={next} className={`absolute right-3 top-1/2 -translate-y-1/2 z-10 opacity-0 group-hover:opacity-100 transition-opacity ${btnClass}`}>
          ...
          >
            ...
            {images.length > 1 && (
              <button
                ...
                className="absolute left-6 top-1/2 -translate-y-1/2 w-14 h-14 rounded-full glass-card flex items-center justify-center hover:scale-105 transition-transform z-10"
              >
            ...
            {images.length > 1 && (
              <button
                ...
                className="absolute right-6 top-1/2 -translate-y-1/2 w-14 h-14 rounded-full glass-card flex items-center justify-center hover:scale-105 transition-transform z-10"
              >
